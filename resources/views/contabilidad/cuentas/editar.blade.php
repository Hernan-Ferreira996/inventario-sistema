@extends('layouts.app')
@section('titulo','Editar Cuenta Contable')
@section('contenido')
<div class="row justify-content-center"><div class="col-lg-7">
<div class="card"><div class="card-header fw-semibold">Editar: {{ $cuenta->codigo }} - {{ $cuenta->nombre }}</div><div class="card-body">
<form method="POST" action="{{ route('contabilidad.cuentas.update',$cuenta) }}">@csrf @method('PATCH')
<div class="row g-3">
    <div class="col-md-4"><label class="form-label fw-semibold">Código</label>
        <input type="text" class="form-control" value="{{ $cuenta->codigo }}" disabled>
        <small class="text-muted">El código no se puede modificar.</small></div>
    <div class="col-md-8"><label class="form-label fw-semibold">Nombre *</label>
        <input type="text" name="nombre" class="form-control" value="{{ old('nombre',$cuenta->nombre) }}" required></div>
    <div class="col-md-4">
        <label class="form-label fw-semibold">Tipo *</label>
        <select name="tipo" class="form-select" required>
            @foreach(\App\Models\CatalogoValor::paraGrupo('cuentas_contables.tipo') as $t)
            <option value="{{ $t->codigo }}" {{ $cuenta->tipo === $t->codigo ? 'selected' : '' }}>{{ $t->etiqueta }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-4">
        <label class="form-label fw-semibold">Naturaleza *</label>
        <select name="naturaleza" class="form-select" required>
            <option value="deudora" {{ $cuenta->naturaleza==='deudora'?'selected':'' }}>Deudora</option>
            <option value="acreedora" {{ $cuenta->naturaleza==='acreedora'?'selected':'' }}>Acreedora</option>
        </select>
    </div>
    <div class="col-md-4">
        <label class="form-label fw-semibold">Cuenta Padre</label>
        <select name="cuenta_padre_id" class="form-select">
            <option value="">-- Ninguna --</option>
            @foreach($cuentasPadre as $p)
            <option value="{{ $p->id }}" {{ $cuenta->cuenta_padre_id == $p->id ? 'selected' : '' }}>{{ $p->codigo }} - {{ $p->nombre }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-12"><div class="form-check">
        <input type="checkbox" name="activo" value="1" class="form-check-input" {{ $cuenta->activo ? 'checked' : '' }}>
        <label class="form-check-label">Cuenta activa</label>
    </div></div>
</div>
<div class="d-flex gap-2 mt-4">
    <button type="submit" class="btn btn-primary"><i class="bi bi-save me-1"></i>Actualizar</button>
    <a href="{{ route('contabilidad.cuentas.index') }}" class="btn btn-outline-secondary">Cancelar</a>
</div>
</form>
</div></div>
</div></div>
@endsection
